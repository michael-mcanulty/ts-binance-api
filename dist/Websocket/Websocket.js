...
class Websocket extends Rest_1.Rest {
	constructor(options) {
		super(options);
		this._reconOptions = {};
		...
		this._reconOptions.constructor = typeof window !== 'undefined' ? Websocket : Html5WebSocket;
	...
	static heartbeat() {
		setInterval(() => __awaiter(this, void 0, void 0, function* () {
			try {
				this.isAlive = yield Websocket.Instance.ping();
			}
			catch (err) {
				let error = new HttpError_1.HttpError({msg: 'DISCONNECTED', code: -1001});
				Websocket._ws.close(error.code, error.message);
			}
		}), 3000);
	}
	...
	openWebSocket(url) {
		if (url) {
			this.url = url;
			Websocket._ws = new ReconnectingWebSocket_1.default(this.url, undefined, this._reconOptions);
			return Websocket._ws;
		}
	}
