...
class BinanceWebsocket extends Rest_1.Rest {
	constructor(options) {
		super(options);
		this._reconOptions = {};
		...
		this._reconOptions.constructor = typeof window !== 'undefined' ? BinanceWebsocket : Html5WebSocket;
	...
	static heartbeat() {
		setInterval(() => __awaiter(this, void 0, void 0, function* () {
			try {
				this.isAlive = yield BinanceWebsocket.Instance.ping();
			}
			catch (err) {
				let error = new HttpError_1.HttpError({msg: 'DISCONNECTED', code: -1001});
				BinanceWebsocket._ws.close(error.code, error.message);
			}
		}), 3000);
	}
	...
	openWebSocket(url) {
		if (url) {
			this.url = url;
			BinanceWebsocket._ws = new ReconnectingWebSocket_1.default(this.url, undefined, this._reconOptions);
			return BinanceWebsocket._ws;
		}
	}
...
BinanceWebsocket.isAlive = false;
